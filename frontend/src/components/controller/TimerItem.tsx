...
export function TimerItem({ timer, index, isActive, onStart, onPause, onReset, onUpdate, onDelete }: TimerItemProps) {
  ...
  const accentColor = isActive ? timerColor : '#2A2A2A'
  ...
  return (
    ...
    >
      ...
      <div className={`border rounded-xl overflow-hidden transition-all duration-200 ${
        ...
        style={isActive ? { boxShadow: `0 4px 24px ${timerColor}15, 0 0 0 1px ${timerColor}20` } : undefined}
      >
        {/* Progress bar */}
        {isActive && (
        ...
        <div className="px-3 py-2.5">
          <div className="flex items-center gap-2">
            ...
            <button
              {...listeners}
              {...attributes}
              className="cursor-grab active:cursor-grabbing text-tm-subtle hover:text-tm-muted transition-colors flex-shrink-0 p-0.5"
            >
          ...
          {/* Expanded section */}
          {expanded && (
            <div className="mt-3 pt-3 border-t border-tm-border grid grid-cols-2 gap-2.5">
              <div>
                <label className="text-[10px] text-tm-subtle uppercase tracking-wider block mb-1.5">Speaker</label>
                <input
                  value={timer.speaker}
                  onChange={(e) => onUpdate(timer.id, { speaker: e.target.value })}
              ...
              <div>
                <label className="text-[10px] text-tm-subtle uppercase tracking-wider block mb-1.5">Notes</label>
                <input
                  value={timer.notes}
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => onUpdate(timer.id, {
                    wrapupColors: { ...timer.wrapupColors, stage3: { ...timer.wrapupColors.stage3, threshold: parseInt(e.target.value) || 60 } }
                  })}
                  className="input-premium w-full py-1.5 text-xs"
                />
              </div>
